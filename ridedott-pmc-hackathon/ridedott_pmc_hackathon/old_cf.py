...
def get_prompt_for_title_extraction() -> str:
    ...
    """
    return "What is the title and subtitle (only if it exists) of the text in one line. Text: {pdf_text}. Title:"


def get_prompt_for_abstract_extraction() -> str:
    """Returns a prompt for abstract extraction.
    ...
    Returns:
        prompt for title extraction, with placeholders for substitution
    """
    return "What is the abstract of the text. Text: {pdf_text}. Abstract:"
...
def extract_abstract_from_text(text: str) -> str:
    """Given the full text of the PDF document, extracts the abstract.
    ...
    if not prompt:
        return ""  # return empty abstract for empty prompt

    response = model.predict(prompt.format(pdf_text=text[:5000]))
    return response.text
...
def extract_summary_from_text(text: str, abstract: str) -> str:
    ...
    prompt = get_prompt_for_summary_of_summaries(summaries)
...
def on_document_added(event, context):
    ...
    abstract = extract_abstract_from_text(complete_text)
...
def get_prompt_for_page_summary_with_context(
    ...
    Page text: {page_text}
    Context of previous pages: {context}
    Abstract of the paper {abstract}
    """
    return prompt_string


def get_prompt_for_summary_of_summaries(context: str) -> str:
    prompt_string = f"""
    Below are the summaries of the pages of a PDF document. Your task is to give me a concise summary
    of the academic paper - what problem is it trying to address, the business context that it's applied to
    , then the main points, and then the conclusion.
    Output only the summary, no other words, avoid section titles like 'summary', 'main points' etc
    Context: {context}"""
    return prompt_string
